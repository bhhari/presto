        ...
        implements SelectiveStreamReader
{
            ...
            throws IOException
    {
        ...
        // account memory used by values, nulls, outputPositions, offsetVector, lengthVector, isNullVector
            ...
            throws IOException
    {
        ...
        boolean isCharType = streamDescriptor.getOrcType().getOrcTypeKind() == OrcType.OrcTypeKind.CHAR;
        int maxCodePointCount = streamDescriptor.getOrcType().getLength().orElse(-1);
    ...
    private void compactValues(int[] positions, int positionCount, boolean includeNulls)
    {
        ...
        for (int i = 0; i < outputPositionCount && positionIndex < positionCount; i++) {
            if (positions[positionIndex] == outputPositions[i]) {
                if (includeNulls) {
