...
import static com.facebook.presto.operator.repartition.BlockEncodingBuffers.createBlockEncodingBuffers;
        ...
        implements Operator
{
    ...
    private static class PagePartitioner
    {
        ...
        private static final Map<String, Integer> FIXED_WIDTH_TYPE_SERIALIZED_BYTES = ImmutableMap.<String, Integer>builder()
                .put(ColorType.NAME, 5)
                .put(StandardTypes.BIGINT, 9)
                .put(StandardTypes.BING_TILE, 9)
                .put(StandardTypes.BOOLEAN, 2)
                .put(StandardTypes.INTEGER, 5)
                .put(StandardTypes.DATE, 5)
                .put(StandardTypes.DOUBLE, 9)
                .put(StandardTypes.INTERVAL_DAY_TO_SECOND, 9)
                .put(StandardTypes.INTERVAL_YEAR_TO_MONTH, 5)
                .put(StandardTypes.IPADDRESS, 17)
                .put(StandardTypes.REAL, 5)
                .put(StandardTypes.SMALLINT, 3)
                .put(StandardTypes.TIME, 9)
                .put(StandardTypes.TIME_WITH_TIME_ZONE, 9)
                .put(StandardTypes.TIMESTAMP, 9)
                .put(StandardTypes.TIMESTAMP_WITH_TIME_ZONE, 9)
                .put(StandardTypes.TINYINT, 2)
                .put(UnknownType.NAME, 2)
                .build();
        ...
        private final BlockFlattener flattener = new BlockFlattener(new SimpleArrayAllocator());
                ...
                Lifespan lifespan)
        {
            ...
            partitionBuffer = new PartitionBuffer[partitionCount];
            for (int i = 0; i < partitionCount; i++) {
                partitionBuffer[i] = new PartitionBuffer(i, sourceTypes.size(), pageSize, pagesAdded, rowsAdded, serde, lifespan);
            }
        ...
        {
            long size = 0;
            for (int i = 0; i < partitionBuffer.length; i++) {
                size += partitionBuffer[i].getInitialRetainedSizeInBytes();
            }
        ...
        public long getRetainedSizeInBytes()
        {
            ...
            for (int i = 0; i < partitionBuffer.length; i++) {
                size += partitionBuffer[i].getRetainedSizeInBytes();
            }
        ...
        public void partitionPage(Page page)
        {
            ...
            for (int i = 0; i < partitionBuffer.length; i++) {
                partitionBuffer[i].resetPositions(positionCount);
            }
            ...
            for (int position = 0; position < positionCount; position++) {
                ...
                if (shouldReplicate) {
                    for (int i = 0; i < partitionBuffer.length; i++) {
                        partitionBuffer[i].addPosition(position);
                    }
                    hasAnyRowBeenReplicated = true;
                }
                else {
                    int partition = partitionFunction.getPartition(partitionFunctionArgs, position);
                    partitionBuffer[partition].addPosition(position);
                }
            }

            // Decode the page just once. The decoded blocks will be fed to each PartitionBuffer object to set up BlockEncodingBuffers.
            for (int i = 0; i < decodedBlocks.length; i++) {
            ...
            for (int i = 0; i < partitionBuffer.length; i++) {
                partitionBuffer[i].appendData(decodedBlocks, fixedWidthRowSize, variableWidthChannels, outputBuffer);
            }
        }
        ...
        {
            for (int i = 0; i < partitionBuffer.length; i++) {
                partitionBuffer[i].flush(outputBuffer);
            }
        }
        ...
        private int getFixedWidthTypeSize(int channel)
        {
            ...
            int bytesPerPosition = FIXED_WIDTH_TYPE_SERIALIZED_BYTES.getOrDefault(type.getDisplayName(), 0);
            if (bytesPerPosition > 0) {
    ...
    private static class PartitionBuffer
    {
        ...
        public long getInitialRetainedSizeInBytes()
        {
            return INSTANCE_SIZE + sizeOf(positions) + sizeOf(serializedRowSizes);
        }
        ...
        public long getRetainedSizeInBytes()
        {
            ...
            // Some destination partitions might get 0 rows. In that case the BlockEncodingBuffers won't be created.
        ...
        private void populateSerializedRowSizes(int fixedWidthRowSize, List<Integer> variableWidthChannels)
        {
            ...
            serializedRowSizes = ensureCapacity(serializedRowSizes, positionCount, SMALL, false, true);
