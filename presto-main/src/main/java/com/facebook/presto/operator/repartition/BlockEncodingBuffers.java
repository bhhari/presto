...
public abstract class BlockEncodingBuffers
{
    ...
    public static BlockEncodingBuffers createBlockEncodingBuffers(DecodedBlockNode decodedBlockNode)
    {
        ...
        // This is because the BlockEncodingBuffers is only created once, while position mapping for Dictionar/Rle blocks
        ...
        if (decodedBlock instanceof ColumnarRow) {
            return new RowBlockEncodingBuffers(decodedBlockNode);
        }
    ...
    public abstract void appendDataInBatch();
     ...
     */
    public abstract void accumulateSerializedRowSizes(int[] serializedRowSizes);

    /**
     * Called by accumulateSerializedRowSizes(int[] serializedRowSizes) for composite types to add row sizes for nested blocks.
     * <p>
     * For example, a BlockEncodingBuffers for an array(int) column with 3 rows - [{1, 2}, {3}, {4, 5, 6}] - will call accumulateSerializedRowSizes
    ...
    protected DecodedBlockNode mapPositionsToNestedBlock(DecodedBlockNode decodedBlockNode)
    {
        ...
        if (decodedObject instanceof RunLengthEncodedBlock) {
            mappedPositions = ensureCapacity(mappedPositions, positionCount, SMALL, false, true);
            positionsMapped = true;
            return decodedBlockNode.getChildren().get(0);
        }
